...
function getFlorOptions(){
  return Array.from(new Set(collectByLevel(ORG_TREE,'ฝ่าย/สำนัก',[]).map(n=>n.name)));
}
...
function ArchNodeModal({title,groupLabel,levelLabel,initial,onClose,onSubmit,codeExample}){
  ...
  const divisionOptions=React.useMemo(getDivisionOptions,[]);
  const unitOptions=React.useMemo(()=>levelLabel==='LEVEL 1 (Work Process)'?getFlorOptions():Array.from(new Set(collectByLevel(ORG_TREE,'กอง/ร.ร.ช่าง กฟภ./สนง.',[]).map(n=>n.name))),[levelLabel]);
...
function ArchLevel2Row({node,l0,l1,group,onEdit,onDelete}){
  ...
  const [status,setStatus]=React.useState(node.status);
  const unitOptions=React.useMemo(()=>Array.from(new Set(collectByLevel(ORG_TREE,'กอง/ร.ร.ช่าง กฟภ./สนง.',[]).map(n=>n.name))),[]);
